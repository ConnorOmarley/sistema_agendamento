  ...
  if (carregando) {
    ...
    return (
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Carregando painel de métricas estruturadas...</p>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-16">
      ...
      <nav className="bg-white shadow-sm border-b px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex gap-6 text-xs font-bold text-gray-500">
            ...
            <Link href="/dashboard/evolucoes" className="hover:text-gray-900">Prontuários / Evoluções</Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={async () => { await supabase.auth.signOut(); router.push('/login'); }} className="text-xs font-bold text-red-600 hover:underline">Sair</button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div>
          ...
          <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider">Visão Geral do Negócio</span>

          <h2 className="text-xl font-extrabold text-gray-900 mt-0.5">Métricas de Desempenho</h2>
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl p-6 text-white shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">

          <div>

            <h3 className="font-extrabold text-base">✨ Novo: Histórico Clínico de Prontuários Centralizado</h3>

            <p className="text-xs text-indigo-100 mt-1">Acesse a central única para ver e filtrar os relatórios de evolução de todos os alunos simultaneamente.</p>

          </div>

          <Link href="/dashboard/evolucoes" className="bg-white text-indigo-700 px-4 py-2 rounded-lg text-xs font-bold hover:bg-indigo-50 transition-all shadow-sm">👁️ Ver Prontuários Coletivos</Link>
        ...
        <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="p-6 border-b flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <form onSubmit={handleCriarAluno} className="w-full sm:w-auto flex gap-2">
              ...
              <input type="text" required placeholder="Nome do novo aluno..." value={nomeNovoAluno} onChange={(e) => setNomeNovoAluno(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-1.5 text-xs focus:outline-none focus:border-indigo-500 w-full sm:w-64" />

              <button type="submit" className="bg-indigo-600 text-white font-bold px-4 py-1.5 rounded-lg text-xs hover:bg-indigo-700 transition-colors shadow-sm">+ Adicionar Novo Aluno</button>
          ...
          <div className="px-6 py-3 bg-gray-50 border-b">
            ...
            <input type="text" placeholder="🔍 Buscar aluno cadastrado..." value={busca} onChange={(e) => setBusca(e.target.value)} className="max-w-md w-full rounded-lg border bg-white border-gray-300 px-3 py-1.5 text-xs focus:outline-none focus:border-indigo-500" />
                ...
                {alunosFiltrados.map((aluno) => (
                  ...
                  <tr key={aluno.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <Link href={`/dashboard/alunos/${aluno.id}`} className="bg-gray-100 hover:bg-indigo-50 hover:text-indigo-700 text-gray-600 font-bold px-3 py-1 rounded-md border transition-all">Ver Perfil Completo</Link>
